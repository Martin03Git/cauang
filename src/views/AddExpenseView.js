...
class AddExpenseView {
    ...
    this.container.innerHTML = `
      ...
      <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-4">
        ...
        <div class="flex items-center border-b-2 border-indigo-500 pb-2">
          ...
          <input id="add-amount" type="number" inputmode="numeric" min="0" placeholder="0"
            class="flex-1 text-3xl font-bold text-gray-800 outline-none border-none bg-transparent [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            autofocus />
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-4">
        ...
        <input id="add-desc" type="text" placeholder="Mis: Nasi padang, bensin, ..."
      ...
      <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-4">
        ...
        <div class="grid grid-cols-4 gap-2">
          ${CATEGORIES.map(cat => `
            <button class="cat-btn flex flex-col items-center gap-1.5 py-3 px-1 rounded-xl border-2 border-gray-100 hover:border-indigo-200 transition-all cursor-pointer"
      ...
      <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-6">
        ...
        <input id="add-date" type="date" value="${getTodayStr()}"
          class="w-full text-sm text-gray-700 outline-none border-b border-gray-200 pb-2 focus:border-indigo-400 transition-colors bg-transparent" />
      </div>
  ...
  attachEvents() {
    document.getElementById('add-back').addEventListener('click', () => this.onSave && this.onSave());
